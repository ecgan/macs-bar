...
./build-app.sh
```
*This script compiles the release bundle, embeds `Sparkle.framework`, sets up RPath, and signs the app using the `CODESIGN_IDENTITY` specified in your local `build.config`.*
...
```
*(Adjust `/path/to/sparkle/bin/` to point to the location of Sparkle's tools on your machine).*
